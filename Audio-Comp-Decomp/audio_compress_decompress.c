...
int encode_wave_file(char *input_file_name, char *output_file_name, bool encodeType)
{
    ...
    // Wave (Audio Format) :  1=PCM; 7=MuLaw
...
void compress_wave_data(unsigned char *inputfile_data_buffer, unsigned char *output_file_data_buffer, int overall_size)
{
    ...
    // Iterate over the input data buffer in chunks of 8 bytes (4 samples of 16-bit each)
    for (int i = 0; i < overall_size; i += 8) {
        
        // Convert pairs of bytes into 16-bit signed integers (samples)
        ...
        // Compress each 16-bit sample into an 8-bit codeword using μ-law compression
        ...
        // Store the 4 compressed codewords into the output buffer
...
{
    int16_t input_data1, input_data2, input_data3, input_data4;
    int16_t codeword1, codeword2, codeword3, codeword4;
